...
class DefinabilityChecker {
 ...
 public:
  DefinabilityChecker(std::vector<int>& universal_variables, const std::vector<int>& existential_variables, std::vector<Clause>& matrix, int& last_used_variable, bool compress=false);
  DefinabilityChecker(DefinabilityChecker&& checker, int& last_used_variable);	
 ...
 private:
  ...
  std::shared_ptr<CadicalSolver> backbone_solver, fast_solver;
